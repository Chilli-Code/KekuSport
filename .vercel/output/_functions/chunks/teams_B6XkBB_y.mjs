import { b as createAstro, c as createComponent, m as maybeRenderHead, d as addAttribute, e as renderScript, a as renderTemplate, r as renderComponent, f as renderTransition } from './astro/server_DpPHlMO2.mjs';
import 'kleur/colors';
import 'clsx';
/* empty css                            */
/* empty css                            */
...
const $$BoxerCard = createComponent(($$result, $$props, $$slots) => {
  ...
  const { id, name, versus, class: extraClass, cardLink, cardImage } = Astro2.props;
...
const $$FighterSelector = createComponent(($$result, $$props, $$slots) => {
  ...
  const { id, name, boxerCardClass, versus, cardLink, cardImage, bigImageSrc } = Astro2.props;
  return renderTemplate`${maybeRenderHead()}<div> ${renderComponent($$result, "BoxerCard", $$BoxerCard, { "id": id, "name": name, "class": `peer ${boxerCardClass}`, "versus": versus, "cardLink": cardLink, "cardImage": cardImage })} <div id="fighter-container" class="pointer-events-none absolute inset-0 flex-col items-center hidden peer-hover:flex peer-focus:flex peer-focus-visible:flex"> <div class="relative top-96 mx-auto h-[4.5rem] flex w-full items-center justify-center z-1 animate-zoom-in"> <!-- <img
        ...
        fetchpriority='low'
      /> --> <div class="relative"> <h3 class="text-7xl font-semibold tracking-wide drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] bg-gradient-to-r from-purple-500 to-emerald-500 bg-clip-text text-transparent relative z-10"> ${name} </h3> <div class="absolute inset-0 bg-emerald-500/80 blur-2xl -z-0 opacity-50 group-hover:opacity-100 transition-opacity"></div> </div> </div> <div class="mask-fade-bottom relative bottom-0 mx-auto h-[80vh] flex w-full items-center justify-center animate-slide-up-fade"> <img${addAttribute(`hero-image-${id}`, "data-id")}${addAttribute(bigImageSrc || `/images/fighters/big/jugador_mistery.webp`, "src")}${addAttribute(name, "alt")} decoding="async" class="w-auto h-full absolute" fetchpriority="low"${addAttribute(renderTransition($$result, "pxwu2rux", "", `image-${id}`), "data-astro-transition-scope")}> </div> </div> </div>`;
}, "C:/Users/jorge/OneDrive/Desktop/pruebaVelada/la-velada-clone/src/components/FighterSelector.astro", "self");

const TEAMS = [
  { id: "peereira", name: "Peereira FC" },
  { id: "rivaldios", name: "Rivaldios FC" },
  { id: "djmariio", name: "DjMaRIIO" },
  { id: "ibai", name: "Ibai Llanos" },
  { id: "grefg", name: "ElGrefg" },
  { id: "spursito", name: "Spursito" },
  { id: "xocas", name: "Xocas" },
  { id: "carreraa", name: "Carreraa" }
];

export { $$FighterSelector as $, TEAMS as T };
